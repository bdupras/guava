...
public class CuckooFilterTest extends TestCase {
  ...
  public void testBasic() {
    for (double fpr = 0.0000001; fpr < 0.1; fpr *= 10) {
      for (int expectedInsertions = 1; expectedInsertions <= 10000; expectedInsertions *= 10) {
        final CuckooFilter<Object> cf = CuckooFilter.create(HashTestUtils.BAD_FUNNEL,
  ...
  public void testOptimalSize() {
    for (int n = 1; n < 1000; n++) {
      for (double fpp = CuckooFilter.MIN_FPP; fpp <= CuckooFilter.MAX_FPP; fpp += 0.001) {
        ...
        assertTrue(CuckooFilter.optimalNumberOfBuckets(n, CuckooFilter.optimalEntriesPerBucket(fpp)) >= 2);
        assertTrue(CuckooFilter.optimalBitsPerEntry(fpp, CuckooFilter.optimalEntriesPerBucket(fpp)) >= 2);
      }
    }
    ...
    try {
      CuckooFilter.create(HashTestUtils.BAD_FUNNEL, Integer.MAX_VALUE, Double.MIN_VALUE);
  ...
  public void testExpectedFpp() {
    CuckooFilter<Object> cf = CuckooFilter.create(HashTestUtils.BAD_FUNNEL, 10, 0.03);
